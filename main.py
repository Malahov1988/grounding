...
class MainWidget(QWidget):
    ...
    def ation_climat_CB(self):
        lst = [self.widget_1.dict_LE['Длинна вертикального заземлителя l, м'].text(),
               self.widget_1.dict_LE['Длина горизонтального заземлителя (сторона 1) a1, м'].text(),
               self.widget_1.dict_LE['Длина горизонтального заземлителя (сторона 2) a2, м'].text()]
        if self.valid_1(lst):
    ...
    def showEvent(self, showEvent):
        ...
        if self.count <= 1:
            if os.path.exists(save_initial):
                ...
                self.widget_1.set_param(
                    [250, 30, 2.5, 10, 0.012, 8, 14, 0.040, 0.8, 2, 1.2, 2.5, 'круглый прокат', 'полосовой прокат',
                     'по контуру', '1'])
...
class Widget_in(QGroupBox):
    ...
    combox_name = ['Тип вертикального заземлителя', 'Тип горизонтального заземлителя', 'Расположение заземлителя',
                   'Климатическая зона']
    sumbol = '*'
    ...
    def add_LE(self):
        ...
        for k, v in self.dict_CB.items():
            ...
            count += 1
        self.dict_CB['Тип вертикального заземлителя'].addItems(
            ['Круглый прокат', 'Уголок стальной', 'Полосовой прокат'])
        self.dict_CB['Тип горизонтального заземлителя'].addItems(
            ['Круглый прокат', 'Уголок стальной', 'Полосовой прокат'])
    ...
    def action_QB_rasp(self):
        if self.dict_CB['Расположение заземлителя'].currentText() == 'По контуру':
            self.dict_labels['Длина горизонтального заземлителя (сторона 1) a1, м'].setText(
                'Длина горизонтального заземлителя (сторона 1) a1, м')
